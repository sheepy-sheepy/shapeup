...
import 'package:shapeup/features/analytics/domain/entities/analytics_chart_point_entity.dart';
import 'package:shapeup/features/measurements/domain/entities/body_measurement_entity.dart';
export 'package:shapeup/features/analytics/domain/usecases/load_analytics_usecase.dart' show AnalyticsDataEntity;
...
class AnalyticsChartController {
  ...
  double value(BodyMeasurementEntity measurement) {
    switch (type) {
      case 'bodyFat':
  ...
  List<AnalyticsChartPointEntity> points() {
    switch (period) {
      case 'month':
  ...
  List<AnalyticsChartPointEntity> _buildWeekPoints() {
    final monday = anchorDate();

    final byDay = <String, BodyMeasurementEntity>{
    ...
    return List.generate(7, (index) {
      ...
      return AnalyticsChartPointEntity(
        label: shortDay(date),
  ...
  List<AnalyticsChartPointEntity> _buildMonthPoints() {
    final anchor = anchorDate();
  ...
  List<AnalyticsChartPointEntity> _buildYearPoints() {
    final anchor = anchorDate();
    final currentYear = anchor.year;
    final points = <AnalyticsChartPointEntity>[];
